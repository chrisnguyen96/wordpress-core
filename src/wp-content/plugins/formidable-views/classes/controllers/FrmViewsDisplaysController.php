...
class FrmViewsDisplaysController {
		...
		switch ( $column_name ) {
			...
			case 'show_count':
				...
				if ( 'calendar' === $show_count ) {
					...
					$post         = get_post( $id );
					$is_grid_type = FrmViewsEditorController::is_grid_type( $post );
					if ( $is_grid_type ) {
	...
	/**
	 * Content with a layout is serialized as an array.
	 ...
	 * @return string
	 */
	private static function get_content_from_first_available_box( $content ) {
		$output = '';
	...
	private static function prepare_content_for_custom_column( $content ) {
		...
		if ( FrmViewsAppHelper::unserialized_content_is_grid_format( $unserialized ) ) {
			$content = self::get_content_from_first_available_box( $unserialized );
		}
		return self::strip_tags_and_truncate( $content );
	}
	...
	public static function get_inner_content_for_listing_page( $view, $args ) {
		...
		$unserialized_content = maybe_unserialize( $unfiltered_content );

		$is_grid_view = FrmViewsAppHelper::unserialized_content_is_grid_format( $unserialized_content );
		if ( $is_grid_view ) {
			$helper             = new FrmViewsLayoutHelper( $view );
			$unfiltered_content = $helper->flatten( $unserialized_content, 'listing' );
		...
		if ( $is_grid_view ) {
			$inner_content = self::apply_grid_container_to_view( $view->ID, $inner_content, $unserialized_content );
		}
	 ...
	 * @param array  $unserialized_content
	 * @return string
	 */
	private static function apply_grid_container_to_view( $view_id, $inner_content, $unserialized_content ) {
		...
		foreach ( $unserialized_content as $box_data ) {
	...
	private static function prepare_grid_classes( $options ) {
		...
		if ( is_array( $options ) && array_key_exists( 'grid_classes', $options ) ) {
	...
	private static function get_inner_content_for_detail_page( $view, $entry_id ) {
		...
		$unserialized_content = maybe_unserialize( $new_content );
		if ( FrmViewsAppHelper::unserialized_content_is_grid_format( $unserialized_content ) ) {
			$helper      = new FrmViewsLayoutHelper( $view );
			$new_content = $helper->flatten( $unserialized_content, 'detail' );
		}
	 ...
	 * @param WP_Post $post
	 */
	private static function maybe_update_post_content( $post ) {
		$content = maybe_unserialize( $post->post_content );
		if ( ! FrmViewsAppHelper::unserialized_content_is_grid_format( $content ) ) {
			return;
		}
		$helper = new FrmViewsLayoutHelper( $post );
		wp_update_post(
			array(
				'ID'           => $post->ID,
				'post_content' => $helper->flatten( $content, 'listing' ),
			)
		);
	}
	 ...
	 * @return string
	 */
	public static function use_first_box_for_excerpt_for_grid( $excerpt, $post ) {
		if ( self::$post_type === $post->post_type && ! $post->post_excerpt ) {
			$unserialized_content = maybe_unserialize( $post->post_content );
			if ( FrmViewsAppHelper::unserialized_content_is_grid_format( $unserialized_content ) ) {
				$content_from_first_available_box = self::get_content_from_first_available_box( $unserialized_content );
				return self::strip_tags_and_truncate( $content_from_first_available_box );
			}
		}
		return $excerpt;
	}
